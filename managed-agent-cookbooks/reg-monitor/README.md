...
Regulatory feed content (Federal Register entries, agency RSS posts, TR alerts) is **untrusted input.** Three-tier isolation:
...
- **Set the Thomson Reuters MCP URL (optional).** TR is commented out in the manifest; wire it and flip `enabled: true` if your team pays for it.
