# Jurisdiction Expansion Skill — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a project-level skill (`.claude/skills/jurisdiction-expansion/SKILL.md`) that guides adaptation of new practice areas for the SA overlay through a structured, research-assisted interview.

**Architecture:** Single SKILL.md file containing the 8-step interview skeleton, research-first pattern, output artifact templates, and state management. The skill loads `jurisdictions/za/ARCHITECTURE.md` and the target plugin's skills as context, runs the interview, writes a decision spec, then invokes `writing-plans`.
...
| File | Responsibility |
|---|---|
| `.claude/skills/jurisdiction-expansion/SKILL.md` | The complete skill — frontmatter, context loading, 8-step interview, research pattern, output templates, state management |

One file to create. No files to modify.

---

## Task 1: Skill Skeleton — Frontmatter, Context Loading, Interview Overview

**Files:**
- Create: `.claude/skills/jurisdiction-expansion/SKILL.md`

- [ ] **Step 1: Create the skill directory**

```bash
mkdir -p .claude/skills/jurisdiction-expansion
```

- [ ] **Step 2: Write the SKILL.md with frontmatter and context loading**

Create `.claude/skills/jurisdiction-expansion/SKILL.md` with this content:
...
1. **Summarise decisions** in a table (like the employment-legal 21-decision table):
...
Employment-legal is the reference implementation. The skill produces the same artifacts for new practice areas.
````

- [ ] **Step 3: Verify the frontmatter is valid**

Run: `head -12 .claude/skills/jurisdiction-expansion/SKILL.md`
Expected: Valid YAML frontmatter with `name`, `description`, and `argument-hint`

- [ ] **Step 4: Verify the file references resolve**

Run: `grep -oP '(?<=Read |read )[`]([^`]+)[`]' .claude/skills/jurisdiction-expansion/SKILL.md | sort -u`

Check each referenced file exists:
```bash
test -f jurisdictions/za/ARCHITECTURE.md && echo "ARCHITECTURE.md: OK"
test -f project/decisions/001-sa-overlay-architecture.md && echo "ADR: OK"
test -f project/prd-sa-employment-overlay.md && echo "PRD: OK"
```
Expected: All OK

- [ ] **Step 5: Verify skill is discoverable**

Run: `ls -la .claude/skills/jurisdiction-expansion/SKILL.md`
Expected: File exists and is readable

- [ ] **Step 6: Commit**

```bash
git add .claude/skills/jurisdiction-expansion/SKILL.md
git commit -m "feat(za): add jurisdiction-expansion skill for adapting new practice areas"
```
...
**Spec coverage:**
- [x] Skill identity (name, location, invocation, argument) — in frontmatter + opening section
- [x] Context loaded at start (ARCHITECTURE.md, ADR, statutes, prior work) — "Before you begin" section
- [x] 8-step interview skeleton — Steps 1-8, each with opening question and adaptive depth
- [x] Research-first pattern (Perplexity → propose → confirm) — dedicated section + integrated into Steps 2, 3, 4, 7
- [x] Source attribution tags — in research-first section
- [x] Output artifacts (decision spec + implementation plan) — "After the interview" section
- [x] State management / pause and resume — dedicated section
- [x] Inherited architecture (not re-decided) — dedicated section citing ADR-001
- [x] Constraints (no US concepts, one upstream change, temporal fields, expert review) — in inherited architecture + interview steps
- [x] Fallback when Perplexity not connected — in research-first section

**Placeholder scan:** No TBDs, TODOs, or "implement later" found. All sections contain complete instructions.

**Consistency check:** File paths consistent throughout (e.g., `jurisdictions/za/{plugin}/`, `docs/superpowers/specs/YYYY-MM-DD-za-{plugin}-expansion.md`). Step numbering is sequential 1-8. Decision table format matches the employment-legal precedent.
